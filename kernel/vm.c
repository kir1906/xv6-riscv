...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for (int level = 2; level > 0; level--)
  {
    pte_t *pte = &pagetable[PX(level, va)]; // PX will return the 9 bits of that level
                                            ...
                                            // current pagetable
    if (*pte & PTE_V)
    {                                        // checks if the entry exsists or not
      pagetable = (pagetable_t)PTE2PA(*pte); // PTE2PA will get that pa without flags and offset bits
    }
    else
    {
      if (!alloc || (pagetable = (pde_t *)kalloc()) == 0) // Crearting new pagetable if it doesn not exsits
      // And have permission to do it
      { // this checks that if it's allowed to add a
...
int mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  if ((va % PGSIZE) != 0) //  va should be the first va of that page
                          // Because we are mapping pages in Pagetable
  ...
  // Kind of last page's virtual address
  for (;;)
  {
    if ((pte = walk(pagetable, a, 1)) == 0) // This adds the new pagetable and ptes throughout all levels
      return -1;
    if (*pte & PTE_V) // Level 0's pte should not be Valid , Valid means it is already in use
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V; // PA2PTE adds flags and removes the offset bits from pa(Physical address )

    if (a == last)
      break;
    a += PGSIZE;  // Jumps to next page
    pa += PGSIZE; // Jumps to next Physical frame
  }
  return 0;
}
...
uvmcreate()
{
  ...
  pagetable = (pagetable_t)kalloc(); // one page for the page table
  if (pagetable == 0)
    return 0;
  memset(pagetable, 0, PGSIZE);
  return pagetable;
...
void freewalk(pagetable_t pagetable)
{
  ...
  for (int i = 0; i < 512; i++)
  {
    ...
    if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0) // Checking for the leaf nodes
    {
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
      freewalk((pagetable_t)child); // Free walking untill we reach to the leaf node
      pagetable[i] = 0;
    }
    else if (pte & PTE_V)
    {
