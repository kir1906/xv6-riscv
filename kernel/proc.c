...
void proc_mapstacks(pagetable_t kpgtbl)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int)(p - proc)); // Return Virtual Address for Kernel Stack
...
void procinit(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    p->state = UNUSED;                   // Making all process available to use
    p->kstack = KSTACK((int)(p - proc)); // Giving address of the kenerl stack
                                         // Same as as defined in proc_mapstacks
  }
}
